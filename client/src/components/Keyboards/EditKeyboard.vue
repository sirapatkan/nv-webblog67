<template>
  <div>
    <h1>Edit Keyboard</h1>
    <form v-on:submit.prevent="editKeyboard">
      <p>Keyboard Name: <input type="text" v-model="keyboard.keyboard" /></p>
      ...
      <vue-ckeditor
        v-model.lazy="keyboard.keyboardtype"
        :config="config"
        @blur="onBlur($event)"
        @focus="onFocus($event)"
      />
      ...
      <p>
        ...
        <button v-on:click="navigateTo('/keyboards')">Back</button>
      </p>
    </form>
  </div>
</template>
